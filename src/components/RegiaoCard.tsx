    ...
    return (
        <div key={id} className='flex-[0_0_100%] min-w-0 md:flex-[0_0_calc(100%/2)] lg:flex-[0_0_calc(100%/3)] px-2 md:px-3'>
            <article className={`bg-[${cores.fundo || '#1e293b'}] p-0 rounded-2xl text-white space-y-4 h-full flex flex-col`}>
                <div className='overflow-hidden relative tooltip' data-rowid={id}>
                    <Image
                        src={img}
                        alt={title}
                        className='object-cover rounded-t-2xl hover:scale-115 transform transition-all duration-300'
                        width={1000}
                        height={1000}
                    />
                </div>
