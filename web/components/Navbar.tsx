      ...
      <div className={`mx-auto flex justify-between gap-8`}>
        ...
        <div className="lg:hidden">
          <button
            ...
            <span className={`text-white hover:text-${cores?.secundaria || 'violet-500'} absolute transform -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2`}>
              <svg
        ...
        <div className={`fixed top-0 left-0 min-h-screen w-80 bg-[#1e293b] shadow-lg transform transition-transform duration-300 ease-in-out ${isMobile ? "translate-x-0" : "-translate-x-full"} lg:hidden z-50`}>
          <div className="flex flex-row items-center border-b pb-4">
            <Link href="#home" className="cursor-pointer text-gray-100 font-bold text-xl pt-4 ps-4">
              SOMBRAS CELESTES
            </Link>
            <button onClick={toggleMobile} className={`cursor-pointer absolute top-4 right-4 text-gray-100 hover:text-${cores?.secundaria || 'violet-500'}`}>
