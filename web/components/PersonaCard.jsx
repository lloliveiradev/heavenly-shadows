...
const PersonaCard = ({ id, img, icon, titulo, subtitulo, descricao, index, active, handleClick }) => (
    <motion.div
        ...
        {active !== id ? (
            ...
            <div className="absolute bottom-0 p-8 flex justify-start w-full flex-col bg-[rgba(0,0,0,0.6)] rounded-b-[24px]">
                ...
                <Link href={`/coletanea?hash=${btoa(id)}`}>
                    <button type='button' className='mt-4 bg-violet-500 text-white font-bold uppercase py-2 px-4 rounded-[8px] hover:bg-violet-700 transition-[background-color] duration-300 ease-in-out float-end'>
                        Explore
                    </button>
                </Link>
            </div>
