...
export class Firestore {
     ...
     * Updates a record
     */
    async set(id: string, data: { [key: string]: any }, user: object, merge: boolean = true) {
        try {
    ...
    async getWhere(options: fsGetWhereOpt = {}): Promise<Array<any>> {
        ...
        try {
            let query: firebaseAdmin.firestore.Query = this.db.collection(this.collection);
            if (filters?.length) {
                for (const filter of filters) {
                    if (filter.or) {
                        filter.key = typeof filter.key == 'string' ? filter.key.split(',') : (Array.isArray(filter.key) ? filter.key : '');
                        filter.op = typeof filter.op == 'string' ? filter.op.split(',') : (Array.isArray(filter.op) ? filter.op : '');
                        filter.val = typeof filter.val == 'string' ? filter.val.split(',') : (Array.isArray(filter.val) ? filter.val : '');
                        ...
                        await Promise.all((Array.isArray(filter.key) ? filter.key : [filter.key]).map((key: string | string[], i: number) => {
                            if (typeof key === 'string') {
                                queries.push(Filter.where(key, filter.op[i] as firebaseAdmin.firestore.WhereFilterOp, filter.val[i] as string));
                            }
                        }));
                        query = query.where(Filter.or(...queries));
                    } else {
                        const key = Array.isArray(filter.key) ? filter.key.join('.') : filter.key;
                        query = query.where(key, filter.op as firebaseAdmin.firestore.WhereFilterOp, filter.val);
                    };
                };
            };
            ...
            if (orderBy) {
                orderBy.forEach(e => {
                    if (typeof e == 'string') {
                        ...
                        query = query.orderBy(e[0], e[1] as firebaseAdmin.firestore.OrderByDirection);
                    };
                });
            };
     ...
     */
    async aggregate(type: 'avg' | 'count' | 'sum' | Array<string>, filters: fsFilters, fields: Array<string>): Promise<any> {
        try {
            ...
            if (!Array.isArray(filters)) throw new CustomError({ message: 'Params most be an array of objects', status: 400 });
            ...
            if (filters?.length) {
                for (const filter of filters) {
                    if (filter.or) {
                        ...
                        const queries: Array<any> = [];
                        await Promise.all((Array.isArray(filter.key) ? filter.key : [filter.key]).map((key: string, i: number) => {
                            queries.push(Filter.where(key, filter.op[i] as firebaseAdmin.firestore.WhereFilterOp, filter.val[i] as string))
                        }));
                        query = query.where(Filter.or(...queries));
                    } else {
     ...
     */
    async ref(id: string): Promise<firebaseAdmin.firestore.DocumentReference | null> {
        try {
